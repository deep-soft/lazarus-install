...
const pkgs: object = {
  win32: {
    ...
    v4_2: {
        laz64: "lazarus-4.2-fpc-3.2.2-win64.exe",
        laz32: "lazarus-4.2-fpc-3.2.2-cross-i386-win32-win64.exe"
    },
    v4_0: {
        laz64: "lazarus-4.0-fpc-3.2.2-win64.exe",
        laz32: "lazarus-4.0-fpc-3.2.2-cross-i386-win32-win64.exe"
    },
    v3_8: {
...
export class Lazarus {
  ...
  async installLazarus(): Promise<void> {
    ...
    switch (this._LazarusVersion) {
          ...
          case "darwin":
            ...
            try {
              if (fs.existsSync(`${lazLibPath}`)) {
                core.info(
                  `installLazarus - Do not need to update lazbuild symlink`
                );
              } else if (fs.existsSync(`${lazAppPath}`)) {
                core.info(
                  `installLazarus - Updating lazbuild symlink to ${lazAppPath}`
                );
                // Remove bad symlink
      ...
      default:
        throw new Error(
          `getLazarus - Version not available: ${this._LazarusVersion}`
        );
    }
  }
